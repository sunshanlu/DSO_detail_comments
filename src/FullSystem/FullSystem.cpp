...
Vec4 FullSystem::trackNewCoarse(FrameHessian *fh) {
    ...
    if (allFrameHistory.size() == 2)
        ...
        SE3 fh_2_slast =
            slast_2_sprelast; // assumed to be the same as fh_2_slast. // 当前帧到上一帧 = 上一帧和大上一帧的
    ...
    for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
        ...
        bool trackingIsGood =
            coarseTracker->trackNewestCoarse(fh, lastF_2_fh_this, aff_g2l_this, pyrLevelsUsed - 1,
        ...
        if (i != 0) {
            printf("RE-TRACK ATTEMPT %d with initOption %d and start-lvl %d (ab %f %f): %f %f %f %f %f -> %f %f %f %f "
                   "%f \n",
                   i, i, pyrLevelsUsed - 1, aff_g2l_this.a, aff_g2l_this.b, achievedRes[0], achievedRes[1],
                   achievedRes[2], achievedRes[3], achievedRes[4], coarseTracker->lastResiduals[0],
                   coarseTracker->lastResiduals[1], coarseTracker->lastResiduals[2], coarseTracker->lastResiduals[3],
...
void FullSystem::traceNewCoarse(FrameHessian *fh) {
    ...
    for (FrameHessian *host : frameHessians) // go through all active frames
    {
        ...
        Vec2f aff = AffLight::fromToVecExposure(host->ab_exposure, fh->ab_exposure, host->aff_g2l(), fh->aff_g2l())
                        .cast<float>();
    ...
    //trace_skip, 100*trace_skip/(float)trace_total, 			trace_badcondition,
    // 100*trace_badcondition/(float)trace_total, 			trace_oob, 100*trace_oob/(float)trace_total,
    // trace_out, 100*trace_out/(float)trace_total, 			trace_uninitialized,
    ...
    for (FrameHessian *host : frameHessians) // go through all active frames
    {
        ...
        for (unsigned int i = 0; i < host->immaturePoints.size(); i += 1) {
            ...
            if ((u > 0 && v > 0 && u < wG[1] && v < hG[1])) {
                // 距离地图 + 小数点
                float dist =
                    coarseDistanceMap->fwdWarpedIDDistFinal[u + wG[1] * v] + (ptp[0] - floorf((float)(ptp[0])));
        ...
        for (unsigned int i = 0; i < host->pointHessians.size(); i++) {
            ...
            //* 把边缘化的帧上的点, 以及受影响较大的点标记为边缘化or删除
                ...
                //* 不是内点直接扔掉
                    ...
                    // ph->numGoodResiduals, (int)ph->residuals.size());
                }
...
void FullSystem::printLogLine() {
    ...
    if (numsLog != 0) {
        (*numsLog) << allKeyFramesHistory.back()->id << " " << statistics_lastFineTrackRMSE << " "
                   ...
                   << " " << ef->resInL << " " << ef->resInM << " " << statistics_numMargResFwd << " "
                   << statistics_numMargResBwd << " " << statistics_numForceDroppedResFwd << " "
