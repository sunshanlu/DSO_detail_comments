...
public:
    typedef nanoflann::L2_Simple_Adaptor<float, FLANNPointcloud> Adaptor;
    typedef nanoflann::KDTreeSingleIndexAdaptor<Adaptor, FLANNPointcloud, 2> KDTree;
...
struct FLANNPointcloud {
    ...
    FLANNPointcloud(int n = 0, Pnt *p = nullptr)
        : num(n)
        , points(p) {}

    // 返回点云中，点的数目，nanoflann要求的接口
    size_t kdtree_get_point_count() const { return num; }

    // 返回点p1和某个点云之间索引的距离，nanoflann要求的接口
    float kdtree_distance(const float *p1, const size_t idx_p2, size_t) const {
        const float d0 = p1[0] - points[idx_p2].u;
        const float d1 = p1[1] - points[idx_p2].v;
        return d0 * d0 + d1 * d1;
    }

    // 返回第idx个点的第dim维数据, nanoflann要求的接口
    ...
    // nanoflann要求接口，仅求最近邻不需要使用，return false
    template <class BBOX> bool kdtree_get_bbox(BBOX & /* bb */) const { return false; }

    int num;     ///< 点云中的点的数量
    Pnt *points; ///< 点云中的点，自定义的Pnt
};
