 ...
 *  1. 构建各层的能量图像金字塔
 *  2. 构建各层的能量图梯度
 *  3. 构建各层的梯度平方，（能量图/灰度图）主要取决于setting_gammaWeightsPixelSelect配置
 * @param color  光度矫正后的能量图像
 * @param HCalib 当梯度平方的计算需要灰度图像时，这里的Hcalib用于提供G-1函数的倒数
 * @note 在计算梯度时，有个问题，就是x的梯度在0和w-1的位置计算都是不对的，计算时取得并不是同一行
 */
void FrameHessian::makeImages(float *color, CalibHessian *HCalib) {
    // 开辟dIp、absSqiareGrad的存储空间
    ...
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
        ...
        if (lvl > 0) {
            ...
            Eigen::Vector3f *dI_lm = dIp[lvlm1]; ///< 上一层的图像梯度
        ...
        for (int idx = wl; idx < wl * (hl - 1); idx++) {
            ...
            /// 如果这里的setting_gammaWeightsPixelSelect的配置为1，则dabs的内容并不是能量图像上的梯度平方，而是原来灰度图像上的梯度平方
...
void FrameFramePrecalc::set(FrameHessian *host, FrameHessian *target, CalibHessian *HCalib) {
    ...
    // 光度仿射值
    PRE_aff_mode =
        AffLight::fromToVecExposure(host->ab_exposure, target->ab_exposure, host->aff_g2l(), target->aff_g2l())
            .cast<float>();
